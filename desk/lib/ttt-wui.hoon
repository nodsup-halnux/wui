        ...
        |^ 
          ...
          ~?  debugmode  
            ...
            ?+  mark
              ...
              ::  We send no upstate card if the board is ~,
...
::
          ...
          ++  give-http
            |=  [rid=@ta hed=response-header:http dat=(unit octs)]
              ...
              ==
::
          ::We only care about the player, the game state, 
          :: and the board
          +$  delta-state  
            $:  rc=coord
              cp=psymbol
          ...
          ==
::
          ::  Generate our upstate data cell.
          ++  state-delta  
            ...
            |=  [bold=boardmap bnew=boardmap]
              ...
              =/  dlist  (get-diff bold bnew)
                ?~  dlist  !!  ::Should never crash.
                  ...
                  -.i.dlist
::
          ::  [!!!] Remove - we use whatever post-state is now!
          ::  All pre and post state board diffs are
          ::  computed by this base gate.
          ::  Not needed, as we use the post-state board
          ::  if a move has been detected.
          ++  get-diff
            |=  [bold=boardmap bnew=boardmap]
              ^-  (list [coord tsymbol])
            ...
            ::  Normally gases and taps done with ~(
            ::  Irregular forms, but the Precepts demand ow.
            ++  get-set
              |=  aboard=boardmap
              ^-  (set [coord tsymbol])
                  %-  
